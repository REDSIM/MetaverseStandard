...
| --- | --- | --- |
| **[World](./Concepts/Worlds%20and%20Sessions.md)** | World Runtime inside a Client | Supplies the scene and application behavior; may integrate Resources and connect to declared services |
...
| **[Item](./Concepts/Items%20and%20Ownership.md)** | Resource Instance inside a World | Can be placed, held, attached, or otherwise integrated under the World's rules; cannot independently control another participant |
| **[Overlay App](./Concepts/Overlay%20Apps.md)** | Independent sandboxed application alongside Worlds | Has its own principal, lifecycle, storage, and grants; uses approved interfaces to interact with a World |
...
| **Brokers** | Mediate network, files, identity, storage, media capture, and device operations | Check the calling principal, destination, approved scope, and budget; return narrow handles rather than ambient access |
...
| Client ↔ platform providers; provider ↔ provider | Search, account and social operations, saved-data synchronization | [Platform Services](./Concepts/Platform%20Services.md), [Identity and Profiles](./Concepts/Identity%20and%20Profiles.md) |
...
| **Simple shared World** | Content delivery plus the declared shared-session services; message routing, indirect participant-content delivery, and media if used | A custom authoritative simulation server, a dedicated host per participant Avatar, or a global account provider |
| **World with custom authoritative logic** | The relevant content/runtime interfaces and an application backend with explicit authority; reusable relays where chosen | Hosting all content, accounts, media, and simulation with one operator, or adopting one universal gameplay protocol |
