...
| **Recommendation** | A proposed architectural correction; not yet a ratified requirement |
...
- Avatars and Items may contain code; it runs in self-scoped Resource Sandboxes and reaches World state only through validated interfaces.
...
- Replication policy, simulation ticks, prediction, and reconciliation remain application or service choices; they cannot bypass platform security or World authority.
...
| D-017 | Moderation feeds, labels, cross-provider block, and reporting | Local controls first; signed provider feeds later | Medium |
...
| D-019 | Directory search, indexing, discovery metadata, reviews, and reputation | Replaceable directories; public listings and optional content descriptions; deeper indexing optional, restricted results access-checked | Medium |
| D-020 | Network host API, transport Features, web-origin mapping, credentials, lifecycle, and errors | Bounded binary I/O through a Network Broker; binding-specific cross-origin rules; evaluate browser-like request, message, stream, and datagram interfaces | Critical |
| D-021 | Optional compact pose profile and adaptive delivery contract | Independent binary updates first; explicit rig mapping, timing, precision, detail changes, and recovery; replaceable provider policies | High |
| D-022 | Modern web use-case coverage and reuse decisions | Maintain [Web Platform Alignment](./Web%20Platform%20Alignment.md); record direct reuse, adaptation, deferral, and justified exclusions | High |
| D-023 | Platform Service discovery, retrieval, account, directory, and social contracts | Client-native bootstrap before World code; core retrieval first, optional standardized roles next; define bindings, schemas, auth, errors, and federation | Critical for retrieval |
| D-024 | Personal data baseline, Collections, audiences, synchronisation, and extensions | Profiles, friends, saved World/Avatar/Item references; private defaults, explicit sharing, separate Resource rights; inheritance, conflicts, migration, and revocation still open | High |
| D-025 | Cross-World social messaging and portal navigation | Federated recipient-addressed delivery independent of session relays; shared Portal semantics with Client-controlled activation; private invitation handling, message privacy, and lifecycle still open | High |
| D-026 | Optional source and session-region preferences | Independent coarse preferences and discoverable choices; fixed hosts remain valid; routing and allocation stay provider-defined; exact metadata and region naming open | Medium |
| D-027 | Commerce offers, Entitlements, Portable Archives, and restoration | Separate deliverables, rights, and service dependencies; optional detached proofs or scoped online checks; chosen issuer trust; schemas, extensible claims, holder binding, recovery, revocation, account changes, and transfer still open | Medium |
| D-028 | AR presentation, spatial interfaces, and persistent placement | Ordinary World model; separate composition, camera, spatial data, and geolocation; exact reference-space, tracking-loss, anchor, storage, and sharing contracts open; no universal mapping or alignment algorithm | Medium |
