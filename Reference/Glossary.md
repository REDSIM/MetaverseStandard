...
**Entitlement** — Issuer-signed statement granting defined rights to a Resource under stated conditions, optionally bound to a particular holder. Acceptance depends on the verifier's trust and usage policy; it does not by itself prove authorship or legal ownership.
...
**World Presence** — One participant's runtime presence in a particular running World or session, including its reported Avatar and participation state. Several may coexist; this does not imply a stored per-World Avatar preset or public disclosure.
